...
c
      fcut =        -tra(p,q,r,pp,qq,rr,pq,pr,qr)/
     ...
     -     (-1 + q + wq)**2*(-2 + q - wqr - wr)*(-2 + q + wqr - wr)*wr*
     -     (1 - r + wr)*(1 + r + wr)*(p - wpr + wr)*(p + wpr + wr)) + 
     -  ((-2 + p + q - s)*trb(2 - q - s,q,-1 + q + s + wpr,pp,qq,rr,pq,pr,
     -      qr))/
     ...
     -     (-2 + p + q + wpr + wqr)*(-2 + q + wqr - wr)*(-2 + q + wqr + wr))\
     ...
     -     (-2 + q + s + wpr + wr))
      fxn = (4.d0/(pi))*(2.d0/3.d0)*ps*pi*fcut/(-2.*p*q)
      return
      end
